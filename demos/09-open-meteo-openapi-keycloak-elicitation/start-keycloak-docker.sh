...
# Set RESHAPR_KEYCLOAK_FOLLOW_LOGS=false when another script should continue after readiness.
set -euo pipefail
...
if [ "${RESHAPR_KEYCLOAK_FOLLOW_LOGS:-true}" != "true" ]; then
  echo "Log follow disabled. Container keeps running until docker stop ${CONTAINER_NAME}."
  exit 0
fi
